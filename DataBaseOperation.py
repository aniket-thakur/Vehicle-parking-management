...
class DBOperation():
    ...
    def __init__(self):                       #constructor
        ...
        #connecting to Database using mysql.connector.connect() function

        self.mydb=mysql.connector.connect(host="localhost",user=datadic['username'],passwd=datadic['password'],database=datadic['database'])
        # passig PARAM host,Database name, Database username, Database Password

    def CreateTables(self):         #function that will create table for first time user
        cursor=self.mydb.cursor()   # creating first query for admin table
