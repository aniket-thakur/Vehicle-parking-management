...
class InstallWindow(QWidget):
    def __init__(self):                                                      # constructor
        ...
        self.resize(400,200)                                                 # window size height and width

        layout=QVBoxLayout()                                                 # Vertical layout object

        label_db_name=QLabel("Database Name : ")
        label_db_name.setStyleSheet("color:#001;padding:8px 0px;font-size:18px;")
        ...
        self.input_db_name=QLineEdit()                                       # Input Box
        self.input_db_name.setText("vehicle_parking")
        self.input_db_name.setStyleSheet("padding:6px;font-size:18px")

        self.input_db_username=QLineEdit()
        self.input_db_username.setText("vehicle_users")
        self.input_db_username.setStyleSheet("padding:6px;font-size:18x")

        self.input_db_password=QLineEdit()
        self.input_db_password.setText("vehicle_password")
        self.input_db_password.setStyleSheet("padding:6px;font-size:18px")

        self.input_admin_username=QLineEdit()
        self.input_admin_username.setStyleSheet("padding:6px;font-size:18px")
        self.input_admin_password=QLineEdit()
        self.input_admin_password.setStyleSheet("padding:6px;font-size:18px")
        self.input_two_wheeler=QLineEdit()
        self.input_two_wheeler.setStyleSheet("padding:6px;font-size:18px")
        self.input_four_wheeler=QLineEdit()
        self.input_four_wheeler.setStyleSheet("padding:6px;font-size:18px")

        buttonsave=QPushButton("Save")                                       # Button to save credentials
        buttonsave.setStyleSheet("padding:5px;font-size:17px;background:green;color:#fgf")

        self.error_label=QLabel()
        self.error_label.setStyleSheet("color:red")

        layout.addWidget(label_db_name)                  # Adding all My label and Input  box into vertical layout obj
        ...
        buttonsave.clicked.connect(self.showStepInfo)              #connect button to the function

        self.setLayout(layout)                                # set layout to class (Qwidget child install window) layout

    def showStepInfo(self):
        if self.input_db_name.text()=="":
            self.error_label.setText("Please Enter Database Name")
            return
